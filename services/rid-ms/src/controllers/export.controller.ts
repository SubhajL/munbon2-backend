...
export class ExportController {
   ...
   */
  async exportParcelsGeoJSON(req: Request, res: Response) {
    const {
      zone,
      cropType,
      waterDemandMethod,
    ...
    try {
      ...
      logger.error('Error exporting parcels GeoJSON:', error);
   ...
   */
  async exportZonesGeoJSON(req: Request, res: Response) {
    const { includeStats, includeWaterDemand } = req.query;

    try {
      // Get zone boundaries by dissolving parcels
      const query = `
        SELECT 
          zone,
      ...
      logger.error('Error exporting zones GeoJSON:', error);
   ...
   */
  async exportWaterDemandHeatmap(req: Request, res: Response) {
    const { zone, method, date, resolution = 'medium' } = req.query;

    try {
      // Get grid size based on resolution
      const gridSizes = {
        high: 0.001,    // ~111m
        medium: 0.005,  // ~555m
        low: 0.01,      // ~1.1km
      };
      const gridSize = gridSizes[resolution as string] || gridSizes.medium;
      ...
      const params: any[] = [];
      if (zone) params.push(zone);
      if (method && !zone) params.push(method);
      if (method && zone) params.push(method);

      // Note: ST_SquareGrid is a PostGIS 3.1+ function
      // For older versions, we need to create a grid manually
      ...
      // Calculate color scale based on demand intensity
      ...
      logger.error('Error exporting water demand heatmap:', error);
      res.status(500).json({ error: 'Failed to export water demand heatmap' });
    }
  }

  /**
   * Custom export with specific attributes
   */
  async customExport(req: Request, res: Response) {
  ...
  /**
   * Get heatmap color based on normalized value
   */
   ...
   */
  private isValidAttribute(attr: string): boolean {
