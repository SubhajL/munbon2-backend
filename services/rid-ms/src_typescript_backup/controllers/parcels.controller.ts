...
export class ParcelsController {
   ...
   */
  async listParcels(req: Request, res: Response) {
    ...
    try {
      ...
      const sortField = allowedSortFields.includes(sortBy as string) ? sortBy : 'parcel_id';
      ...
      logger.error('Error listing parcels:', error);
  ...
  async searchParcels(req: Request, res: Response) {
    ...
    try {
      const fields = (searchFields as string).split(',').map(f => f.trim());
  ...
  /**
   * Get parcel version history
   */
  async getParcelHistory(req: Request, res: Response) {
    ...
    try {
      ...
      logger.error('Error getting parcel history:', error);
      res.status(500).json({ error: 'Failed to get parcel history' });
    }
  }

  /**
   * Get parcels valid at specific date
   */
  async getParcelsAtDate(req: Request, res: Response) {
    ...
    try {
      ...
      logger.error('Error getting parcels at date:', error);
  ...
  async updateParcel(req: Request, res: Response) {
    ...
    try {
      ...
      const currentResult = await client.query(
        'SELECT * FROM parcels WHERE id = $1 AND valid_to IS NULL',
        [id]
      );
      ...
      await client.query('ROLLBACK');
      logger.error('Error updating parcel:', error);
