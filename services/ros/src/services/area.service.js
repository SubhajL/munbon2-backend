...
class AreaService {
    ...
    async getAreasByType(areaType) {
        ...
        ORDER BY area_id
      `;
            const result = await database_1.pool.query(query, [areaType]);
            return result.rows.map(row => this.mapRowToAreaInfo(row));
        }
        catch (error) {
            logger_1.logger.error('Failed to get areas by type', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getChildAreas(parentAreaId) {
        try {
            const query = `
        SELECT * FROM area_info 
      ...
      `;
            const result = await database_1.pool.query(query, [parentAreaId]);
            return result.rows.map(row => this.mapRowToAreaInfo(row));
        }
        catch (error) {
            logger_1.logger.error('Failed to get child areas', error);
            throw error;
        }
    }
    /**
    ...
    async updateArea(areaId, updates) {
      ...
      `;
            ...
            logger_1.logger.error('Failed to update area', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getAreaHierarchy(projectId) {
        try {
            // Get project
            ...
            logger_1.logger.error('Failed to get area hierarchy', error);
            throw error;
        }
    }
    /**
     ...
     */
    async calculateTotalArea(parentAreaId) {
        try {
            const query = `
      ...
      `;
            ...
            logger_1.logger.error('Failed to calculate total area', error);
            throw error;
        }
    }
    /**
    ...
    async deleteArea(areaId) {
      ...
      `, [areaId]);
            ...
            logger_1.logger.error('Failed to delete area', error);
            throw error;
        }
    }
    /**
    ...
    async importAreas(areas) {
        ...
        for (const area of areas) {
            try {
                ...
                logger_1.logger.error(`Failed to import area ${area.areaId}`, error);
                failed++;
            }
        }
