...
exports.plotWaterDemandService = exports.PlotWaterDemandService = void 0;
const database_1 = require("@config/database");
const logger_1 = require("@utils/logger");
const water_demand_service_1 = require("./water-demand.service");
class PlotWaterDemandService {
    /**
    ...
    async getPlotInfo(plotId) {
        ...
        WHERE plot_id = $1
      `;
            const result = await database_1.pool.query(query, [plotId]);
            ...
            logger_1.logger.error('Failed to get plot info', error);
            throw error;
        }
    }
    /**
     ...
     */
    async calculatePlotWaterDemand(input) {
        try {
            // Get plot information
            ...
            const result = await water_demand_service_1.waterDemandService.calculateSeasonalWaterDemand(input.plotId, 'plot', // area type
            plotInfo.areaRai, input.cropType, input.plantingDate, input.includeRainfall ?? true, input.includeLandPreparation ?? true);
            ...
            logger_1.logger.error('Failed to calculate plot water demand', error);
            throw error;
        }
    }
    /**
     ...
     */
    async calculateBatchPlotWaterDemand(input) {
        try {
            ...
            for (let i = 0; i < input.plotIds.length; i += batchSize) {
                ...
                const batchPromises = batch.map(plotId => this.calculatePlotWaterDemand({
                    plotId,
                    ...
                    .catch(error => {
                    logger_1.logger.error(`Failed to calculate for plot ${plotId}`, error);
                    return null;
                }));
            ...
            logger_1.logger.error('Failed to calculate batch plot water demand', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getPlotsByArea(areaType, areaId) {
        try {
      ...
      `;
            ...
            logger_1.logger.error('Failed to get plots by area', error);
            throw error;
        }
    }
    /**
     ...
     */
    async savePlotSeasonalDemand(plotInfo, input, result) {
        try {
            const query = `
      ...
      `;
            ...
            logger_1.logger.error('Failed to save plot seasonal demand', error);
            throw error;
        }
    }
    /**
     ...
     */
    async savePlotWeeklyDemands(plotInfo, input, result) {
        try {
            if (!result.weeklyDetails)
                return;
        ...
        `;
            ...
            logger_1.logger.error('Failed to save plot weekly demands', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getPlotHistoricalDemand(plotId, startYear, endYear) {
        try {
            let query = `
      ...
      `;
            ...
            logger_1.logger.error('Failed to get plot historical demand', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getCurrentWeekDemandForActivePlots(currentWeek, currentYear) {
        try {
            const query = `
        SELECT 
          pwd.*,
      ...
      `;
            const result = await database_1.pool.query(query, [currentWeek, currentYear]);
            return result.rows;
        }
        catch (error) {
            logger_1.logger.error('Failed to get current week demand', error);
            throw error;
        }
    }
}
exports.PlotWaterDemandService = PlotWaterDemandService;
exports.plotWaterDemandService = new PlotWaterDemandService();
//# sourceMappingURL=plot-water-demand.service.js.map