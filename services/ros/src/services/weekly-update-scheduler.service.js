...
const { waterDemandService } = require('./water-demand.service');
const { waterLevelAggregationService } = require('./water-level-aggregation.service');
...
class WeeklyUpdateSchedulerService {
  constructor() {
   ...
   */
  async getActiveCropsForUpdate(currentDate) {
    try {
      const query = `
   ...
   */
  async recalculateWaterDemandWithWaterLevel(crop, calendarWeek, calendarYear) {
    try {
   ...
   */
  async updateRecalculationTimestamp(areaId, areaType, cropType, cropWeek, calendarWeek, calendarYear) {
    const query = `
   ...
   */
  async storeUpdateSummary(week, year, totalCrops, successCount, failureCount, durationMs) {
    const query = `
   ...
   */
  async runManualUpdate(week, year) {
...
const weeklyUpdateScheduler = new WeeklyUpdateSchedulerService();

module.exports = {
  WeeklyUpdateSchedulerService,
  weeklyUpdateScheduler
};