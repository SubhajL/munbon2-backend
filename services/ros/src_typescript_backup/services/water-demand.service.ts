...
export class WaterDemandService {
  ...
  async calculateWaterDemand(input: WaterDemandInput): Promise<WaterDemandResult> {
    try {
      ...
      // Get weekly aggregated water level
      let weeklyWaterLevel = null;
      let waterLevelAdjustment = null;
      
      // Use provided water level or fetch weekly aggregated
      if (input.waterLevel !== undefined) {
        // If water level is provided directly, use it
        waterLevelAdjustment = await waterLevelAggregationService.calculateAdjustmentFactor(
          input.cropType,
          input.cropWeek,
          input.waterLevel
        );
      } else {
        // Fetch weekly aggregated water level
        weeklyWaterLevel = await waterLevelAggregationService.getWeeklyWaterLevel(
          input.areaId,
          input.areaType,
          input.calendarWeek,
          input.calendarYear
        ...
        if (weeklyWaterLevel && weeklyWaterLevel.avgWaterLevelM !== null) {
          waterLevelAdjustment = await waterLevelAggregationService.calculateAdjustmentFactor(
            input.cropType,
            input.cropWeek,
            weeklyWaterLevel.avgWaterLevelM
          );
        }
      }
      ...
      let adjustedNetDemandMm: number | undefined;
      let adjustedNetDemandM3: number | undefined;
      ...
      if (effectiveRainfall !== undefined) {
        ...
        // Apply water level adjustment if available
        if (waterLevelAdjustment && waterLevelAdjustment.factor !== 1.0) {
          adjustedNetDemandMm = netWaterDemandMm * waterLevelAdjustment.factor;
          adjustedNetDemandM3 = adjustedNetDemandMm * input.areaRai * this.RAI_TO_M3_FACTOR;
        }
      }
      ...
      const result: WaterDemandResult = {
        ...
        cropWaterDemandM3,
        effectiveRainfall,
        waterLevel: weeklyWaterLevel?.avgWaterLevelM || input.waterLevel,
        netWaterDemandMm,
        netWaterDemandM3,
        // New fields for water level integration
        weeklyAvgWaterLevel: weeklyWaterLevel?.avgWaterLevelM,
        waterLevelAdjustmentFactor: waterLevelAdjustment?.factor,
        adjustedNetDemandMm,
        adjustedNetDemandM3,
        waterLevelDataQuality: weeklyWaterLevel?.dataQualityScore || waterLevelAdjustment?.quality,
        adjustmentMethod: waterLevelAdjustment?.method,
      };
    ...
    try {
      const query = `
        INSERT INTO ros.water_demand_calculations (
          ...
          weekly_avg_water_level_m, water_level_adjustment_factor,
          adjusted_net_demand_mm, adjusted_net_demand_m3,
          water_level_data_quality, adjustment_method
        ) VALUES (
      ...
      const values = [
        ...
        result.weeklyAvgWaterLevel || null,
        result.waterLevelAdjustmentFactor || null,
        result.adjustedNetDemandMm || null,
        result.adjustedNetDemandM3 || null,
        result.waterLevelDataQuality || null,
        result.adjustmentMethod || null,
      ];
