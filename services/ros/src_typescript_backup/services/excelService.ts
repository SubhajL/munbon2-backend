...
export class ExcelService {
   ...
   */
  async parseKcData(buffer: Buffer): Promise<Array<{
    cropType: string;
    growthWeek: number;
    kcValue: number;
    growthStage?: string;
    ...
    try {
      ...
      if (!kcSheet) {
        throw new AppError('Kc sheet not found in Excel file', 400);
      }

      const data = XLSX.utils.sheet_to_json(kcSheet, { header: 1 }) as any[][];
      const results = [];
      ...
      logger.info(`Parsed ${results.length} Kc values for ${crops.length} crops`);
      return results;
    } catch (error) {
      logger.error('Error parsing Kc data:', error);
      throw new AppError('Failed to parse Kc data from Excel', 400);
    }
  }
   ...
   */
  async parseET0Data(buffer: Buffer): Promise<Array<{
    location?: string;
    year: number;
    month: number;
    et0Value: number;
    ...
    try {
      ...
      if (!et0Sheet) {
        throw new AppError('ETo sheet not found in Excel file', 400);
      }

      const data = XLSX.utils.sheet_to_json(et0Sheet, { header: 1 }) as any[][];
      const results = [];
      ...
      logger.info(`Parsed ET0 data for ${results.length} months`);
      return results;
    } catch (error) {
      logger.error('Error parsing ET0 data:', error);
      throw new AppError('Failed to parse ET0 data from Excel', 400);
    }
  }
   ...
   */
  async parseRainfallData(buffer: Buffer): Promise<Array<{
    location?: string;
    year: number;
    month: number;
    ...
    try {
      ...
      if (!rainSheet) {
        throw new AppError('Rainfall sheet not found in Excel file', 400);
      }

      const data = XLSX.utils.sheet_to_json(rainSheet, { header: 1 }) as any[][];
      const results = [];
      const currentYear = new Date().getFullYear();
      ...
      logger.info(`Parsed rainfall data for ${results.length} months`);
      return results;
    } catch (error) {
      logger.error('Error parsing rainfall data:', error);
      throw new AppError('Failed to parse rainfall data from Excel', 400);
    }
  }
   ...
   */
  async parseROSExcelFile(buffer: Buffer): Promise<{
    cropType: string;
    ...
    try {
      ...
      if (!fillDataSheet) {
        throw new AppError('fill_data sheet not found', 400);
      }
      ...
      logger.error('Error parsing ROS Excel file:', error);
      throw error;
    }
  }
   ...
   */
  private extractPlantingSchedule(sheet: XLSX.WorkSheet): PlantingData[] {
    if (!sheet) return [];

    const data = XLSX.utils.sheet_to_json(sheet, { header: 1 }) as any[][];
    const schedule: PlantingData[] = [];
   ...
   */
  private getCellValue(sheet: XLSX.WorkSheet, address: string): any {
   ...
   */
  private getNumericCellValue(sheet: XLSX.WorkSheet, address: string): number | undefined {
   ...
   */
  private determineGrowthStage(week: number): string {
    if (week <= 3) return 'initial';
    if (week <= 7) return 'development';
    if (week <= 13) return 'mid-season';
    if (week <= 16) return 'late-season';
    return 'harvest';
  }
