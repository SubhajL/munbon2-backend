...
export class WaterLevelService {
   ...
   */
  async getCurrentWaterLevel(areaId: string): Promise<WaterLevelData | null> {
    try {
      const query = `
        SELECT 
          area_id,
      ...
      logger.error('Failed to get current water level', error);
      throw error;
    }
  }
   ...
   */
  async saveWaterLevel(data: WaterLevelData): Promise<void> {
    try {
      const query = `
      ...
      logger.info('Water level data saved', { 
        areaId: data.areaId, 
        level: data.waterLevelM,
        date: data.measurementDate 
      });
    } catch (error) {
      logger.error('Failed to save water level data', error);
      throw error;
    }
  }
  ...
  async getWaterLevelHistory(
    ...
    try {
      ...
      logger.error('Failed to get water level history', error);
      throw error;
    }
  }
  ...
  async getAverageWaterLevel(
    ...
    try {
      ...
      logger.error('Failed to get average water level', error);
      throw error;
    }
  }
  ...
  async checkCriticalLevel(
    ...
    try {
      ...
      logger.error('Failed to check critical water level', error);
      throw error;
    }
  }
  ...
  async importSensorData(
    ...
    try {
      ...
      logger.info(`Imported ${data.length} water level records from sensors`);
    } catch (error) {
      logger.error('Failed to import sensor data', error);
      throw error;
    }
  }
  ...
  async getWaterLevelStatistics(
    ...
    try {
      ...
      logger.error('Failed to get water level statistics', error);
      throw error;
    }
  }
}
