...
# Deploy ROS service to EC2 (Simple version with ts-node)
set -e
...
echo -e "${BLUE}=== DEPLOYING ROS SERVICE TO EC2 (SIMPLE VERSION) ===${NC}"
...
# Step 2: Build Docker image locally with simple Dockerfile
echo -e "\n${BLUE}Step 1: Building Docker image...${NC}"
docker build -f Dockerfile.simple -t ros-service:latest .
...
docker stop ros-service 2>/dev/null || true
docker rm ros-service 2>/dev/null || true
EOF
...
# Step 6: Update docker-compose.yml to use prebuilt image
echo -e "\n${BLUE}Step 5: Updating docker-compose configuration...${NC}"
ssh -i "$SSH_KEY" $EC2_USER@$EC2_HOST << 'EOF'
cd ~/services/ros
...
# Step 7: Load image and start service on EC2
