...
function extractTestDataFromExcel(excelPath: string): ExtractedTestCase[] {
...
function extractEToData(sheet: XLSX.WorkSheet): Map<number, number> {
  const etoMap = new Map<number, number>();
...
function extractKcData(sheet: XLSX.WorkSheet): Map<string, Map<number, number>> {
  ...
  crops.forEach(crop => {
    const column = columnMap[crop as keyof typeof columnMap];
...
function extractScenarios(
  ...
  etoData: Map<number, number>,
  kcData: Map<string, Map<number, number>>
): ExtractedTestCase[] {
  const scenarios: ExtractedTestCase[] = [];
...
function generateEdgeCaseTests(
  ...
  const edgeCases: ExtractedTestCase[] = [];
...
function saveTestCases(testCases: ExtractedTestCase[], outputPath: string) {
  const output = {
...
function generateTestCode(testCases: ExtractedTestCase[]): string {
  const testCode = `
...
async function main() {
  ...
  const excelPath = args[0] || resolve(__dirname, '../tests/fixtures/ROS_Calculation.xlsx');
  const outputPath = args[1] || resolve(__dirname, '../tests/fixtures/excel-test-cases.json');
  
  console.log(chalk.bold.blue('🔍 ROS Excel Test Data Extractor\n'));
  ...
  try {
    ...
    writeFileSync(testCodePath, testCode);
    console.log(chalk.green(`✅ Generated test code: ${testCodePath}`));
    ...
    console.error(chalk.red('❌ Error extracting test data:'), error);
    process.exit(1);
  }
}
