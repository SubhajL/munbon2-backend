...
function runValidationTests(): ValidationResult[] {
  ...
  const calc2 = calculateWaterDemand(
    THAI_EXCEL_DATA.eto[test2.calendarMonth],
    THAI_EXCEL_DATA.kcRice[test2.cropWeek],
    test2.areaRai
  );
  ...
  const calc3 = calculateWaterDemand(
    THAI_EXCEL_DATA.eto[test3.calendarMonth],
    THAI_EXCEL_DATA.kcRice[test3.cropWeek],
    test3.areaRai
  );
...
function saveValidationReport(results: ValidationResult[]) {
  const report = {
  ...
  const reportPath = resolve(__dirname, '../reports/thai-excel-validation-expected.json');
...
// Main execution
function main() {
  console.log(chalk.bold.blue('🇹🇭 Thai ROS Excel Validation\n'));
