...
interface WeeklyCalculation {
  ...
  cropWaterDemandM3PerRai: number; // Per rai
  effectiveRainfallMm: number;
  netWaterDemandMm: number;
  netWaterDemandM3PerRai: number; // Per rai
}

async function getWeeklyETo(calendarWeek: number, calendarYear: number): Promise<number> {
  ...
  const monthlyResult = await pool.query(monthlyQuery, [month]);
...
async function getKcValue(cropWeek: number): Promise<number> {
  const query = `
    SELECT kc_value 
    FROM ros.kc_weekly 
  ...
  const result = await pool.query(query, [CROP_TYPE, cropWeek]);
  return parseFloat(result.rows[0]?.kc_value) || 0;
}

async function getEffectiveRainfall(month: number): Promise<number> {
  const query = `
  ...
  const result = await pool.query(query, [month, CROP_TYPE]);
  return parseFloat(result.rows[0]?.effective_rainfall_mm) || 0;
}

async function getLandPreparationWater(): Promise<number> {
  const query = `
  ...
  const result = await pool.query(query, [CROP_TYPE]);
  return parseFloat(result.rows[0]?.preparation_water_mm) || 100; // Default 100mm for rice
}

async function calculateWeeklyDemand(cropWeek: number, plantingDate: Date): Promise<WeeklyCalculation> {
...
async function calculateLandPreparation(plantingDate: Date): Promise<WeeklyCalculation> {
  const RAI_TO_M3_FACTOR = 1.6;
  
  // Land preparation is week 0 (1 week before planting)
  const prepDate = dayjs(plantingDate).subtract(1, 'week');
...
async function insertWeeklyDemand(plot: PlotData, calc: WeeklyCalculation, plantingDate: Date): Promise<void> {
  const query = `
...
async function insertSeasonalSummary(
  plot: PlotData, 
  calculations: WeeklyCalculation[], 
  plantingDate: Date
): Promise<void> {
  // Calculate totals
  ...
  const harvestDate = dayjs(plantingDate).add(CROP_WEEKS, 'week').toDate();
...
async function processPlots(): Promise<void> {
  try {
    // Get all plots
    const plotsQuery = `
    ...
    const plotsResult = await pool.query(plotsQuery);
    const plots = plotsResult.rows as PlotData[];
    ...
    const summaryResult = await pool.query(summaryQuery, [CROP_TYPE, plantingDate]);
