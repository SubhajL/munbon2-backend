...
async function listEffectiveRainfallByWeek() {
    ...
    `;
      ...
      `;
            const monthResult = await database_1.pool.query(monthQuery, [month]);
            const monthlyRainfall = parseFloat(monthResult.rows[0]?.effective_rainfall_mm || '0');
            console.log(`  ${cropWeek.toString().padStart(2)} | ` +
    ...
    `;
        ...
        monthlyResult.rows.forEach(row => {
            ...
            const monthName = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][month - 1];
            console.log(`${month.toString().padStart(2)}-${monthName} | ` +
...
//# sourceMappingURL=list-effective-rainfall-by-week.js.map