...
from core.gate_flow import gate_flow_m3s, required_opening_m
...
class HydraulicService:
    ...
    def _build_gate_flow_cal(self, gate_id: str, calibration):
        ...
        Delegates to the single assembly home on the loader (C10) so this service and
        NetworkFlowController never diverge."""
        return self.calibration_loader.build_flow_calibration(gate_id, calibration)

    @staticmethod
    def _max_opening_from_geometry(calibration) -> Optional[float]:
        """Max vertical opening (m) from workbook geometry — delegates to the loader's
        single implementation (2.3-retro HIGH bound lives there)."""
        return GateCalibrationLoader._max_opening_from_geometry(calibration)
