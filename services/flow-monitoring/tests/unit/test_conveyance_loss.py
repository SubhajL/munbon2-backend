...
class TestWettedPerimeter:
    ...
    def test_rectangular_channel_has_no_slope_term(self):
        ...
        assert wetted_perimeter(
            {"bottom_width_m": 8.0, "side_slope": 0.0}, 2.0
        ) == pytest.approx(12.0)
...
class TestReachLossUplift:
    ...
    def test_operational_term_scales_with_throughflow(self):
        ...
        assert reach_loss_uplift(reach, 20.0) - reach_loss_uplift(
            reach, 10.0
        ) == pytest.approx(0.05 * 10.0, rel=1e-9)
    ...
    def test_default_op_fraction_applies_once_per_reach_not_per_segment(
        self, monkeypatch
    ):
    ...
    def test_geometry_is_strict_json(self):
        def _reject(constant):
            raise AssertionError(
                f"non-strict JSON constant {constant!r} in canal_geometry.json"
            )
...
def _survey_row(
    from_node, to_node, from_km, to_km, length, lining="concrete", q_max=None
):
...
class TestSectionsByEdgeFromGeometry:
    def test_parses_all_survey_sections_keyed_by_normalized_edge(self):
        ...
        # First LMC reach runs M(0,0) -> M(0,2) over 0+000-1+620: the flume
        # [0,170] has no cross-section survey, so the reach carries ONE emitted
        # segment — the [170,1620] piece of the 0+170-1+620 concrete row.
        [seg] = sections[("M(0,0)", "M(0,2)")]
        assert seg["length_m"] == 1450
        assert (
            seg["seepage_rate_m_s"] == 1.0e-5
        )  # enriched from lining_type=concrete (aged)
    ...
    def test_duplicate_edge_rows_become_ordered_segments_not_overwrites(self):
        ...
        geo = {
            "canal_sections": [
                ...
                _survey_row(
                    "M(0,1)", "M(0,2)", "0+000", "1+000", 1000, lining="concrete"
                ),
            ]
        }
        sections = sections_by_edge_from_geometry(geo)
    ...
    def test_single_row_inverted_span_fails_closed(self):
        ...
        geo = {
            "canal_sections": [_survey_row("M(0,1)", "M(0,2)", "1+000", "0+500", 500)]
        }
        with pytest.raises(ConfigError, match="span"):
            sections_by_edge_from_geometry(geo)
    ...
    def test_reach_span_exposes_head_and_tail_gaps(self):
        ...
        assert reach_chainage_gap_m(segments, span=(170.0, 500.0)) == pytest.approx(
            200.0
        )
    ...
    def test_reach_span_adds_boundary_to_interior_gaps(self):
        ...
        assert reach_chainage_gap_m(segments, span=(0.0, 1000.0)) == pytest.approx(
            400.0
        )
...
class TestDryReachGating:
    ...
    def test_zero_demand_aggregation_charges_nothing(self):
        ...
        sections = {
            ("S", "M(0,1)"): _reach(1.0e-5, op=0.0),
            self.EDGE: _reach(1.0e-5, op=0.0),
        }
    ...
    def _branch_sections(self, op=0.0):
        return {
            ("M(0,0)", "M(0,1)"): _reach(1.0e-5, op=op),
    ...
    def test_always_wet_charges_ancestors_but_not_the_dry_sibling(self):
        ...
        assert flow[("S", "M(0,0)")] == pytest.approx(
            2 * self.SEEP
        )  # no geometry on S edge

    def test_always_wet_with_operational_fraction_charges_only_flowing_reaches(self):
        rl = make_reach_loss(
            self._branch_sections(op=0.05), always_wet=frozenset({self.WET})
        )
        flow = required_flow_per_reach(self.BRANCH_EDGES, {}, reach_loss=rl)
        assert flow[self.WET] == pytest.approx(
            self.SEEP
        )  # op term is 0.05 * 0 on the wet reach
...
class TestAggregationWithLoss:
    ...
    def test_seepage_only_head_equals_demand_plus_path_seepage(self):
        ...
        flow = required_flow_per_reach(
            self.EDGES, {"M(0,2)": 10.0}, reach_loss=make_reach_loss(sections)
        )
        seep = 3.0e-7 * P * 1000.0
    ...
    def test_losses_lift_head_above_the_lossless_head(self):
        sections = {
            ("M(0,0)", "M(0,1)"): _reach(3.0e-7),
            ("M(0,1)", "M(0,2)"): _reach(3.0e-7),
        }
        demand = {"M(0,2)": 10.0}
        lossy = required_flow_per_reach(
            self.EDGES, demand, reach_loss=make_reach_loss(sections)
        )
