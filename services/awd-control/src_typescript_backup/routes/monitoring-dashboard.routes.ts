...
/**
 * Get real-time monitoring data for active irrigation
 */
router.get('/monitoring/irrigation/:scheduleId/realtime',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        scheduleId,
        dataPoints: result.rows.reverse(), // Chronological order
        count: result.rows.length
      });

    } catch (error) {
      logger.error({ error, scheduleId: req.params.scheduleId }, 'Failed to get realtime data');
...
/**
 * Get irrigation performance summary
 */
router.get('/monitoring/fields/:fieldId/performance',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, fieldId: req.params.fieldId }, 'Failed to get performance summary');
...
router.get('/monitoring/anomalies',
  [
    query('fieldId').optional().isUUID().withMessage('Invalid field ID'),
    query('type').optional().isIn(['low_flow', 'no_rise', 'rapid_drop', 'sensor_failure', 'overflow_risk']),
...
router.get('/monitoring/water-usage',
  [
    query('groupBy').optional().isIn(['field', 'day', 'week', 'month']),
    query('days').optional().isInt({ min: 1, max: 365 })
  ],
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error }, 'Failed to get water usage analytics');
...
router.get('/monitoring/gates/:gateId/history',
  [
    param('gateId').isString().notEmpty().withMessage('Invalid gate ID'),
    query('days').optional().isInt({ min: 1, max: 365 })
  ],
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, gateId: req.params.gateId }, 'Failed to get gate history');
