...
exports.irrigationControllerV2Service = exports.IrrigationControllerV2Service = void 0;
const database_1 = require("../config/database");
const redis_1 = require("../config/redis");
const logger_1 = require("../utils/logger");
const kafka_1 = require("../config/kafka");
const sensor_management_service_1 = require("./sensor-management.service");
const scada_gate_control_service_1 = require("./scada-gate-control.service");
const uuid_1 = require("uuid");
class IrrigationControllerV2Service {
    ...
    async startIrrigation(config) {
        ...
        try {
            ...
            const requiredFlowRate = await this.calculateRequiredFlowRate(config.fieldId, initialLevel.waterLevelCm, config.targetLevelCm);
            await scada_gate_control_service_1.scadaGateControlService.openGateForFlow(config.fieldId, config.targetFlowRate || requiredFlowRate);
    ...
    startMonitoring(scheduleId, config) {
        ...
        const monitoringInterval = setInterval(async () => {
            try {
                ...
                const canalLevels = await scada_gate_control_service_1.scadaGateControlService.getCanalWaterLevels();
    ...
    async calculateRequiredFlowRate(fieldId, currentLevel, targetLevel) {
        try {
      ...
      `, [fieldId]);
            ...
            const depthNeeded = (targetLevel - currentLevel) / 100;
            const volumeNeeded = area_hectares * 10000 * depthNeeded;
            const percolationRate = this.getPercolationRate(soil_type);
            ...
            logger_1.logger.error({ error, fieldId }, 'Failed to calculate flow rate');
            return 5.0;
        }
    }
    getPercolationRate(soilType) {
        const rates = {
            'clay': 1.0,
            'loam': 2.0,
            'sandy_loam': 3.0,
        ...
        return rates[soilType] || 2.0;
    }
    async storeCanalLevels(scheduleId, levels) {
        try {
      ...
      `, [scheduleId, JSON.stringify(levels)]);
            ...
            logger_1.logger.error({ error, scheduleId }, 'Failed to store canal levels');
        }
    }
    ...
    async handleAnomaly(scheduleId, fieldId, anomaly) {
        ...
        switch (anomaly.type) {
            ...
            case 'overflow_risk':
                ...
                break;
            case 'low_flow':
                logger_1.logger.info({ fieldId }, 'Low flow detected - may need gate adjustment');
                break;
            case 'no_rise':
                const canalStatus = await scada_gate_control_service_1.scadaGateControlService.getCanalWaterLevels();
                logger_1.logger.info({ fieldId, canalStatus }, 'Checking canal status due to no rise');
                break;
            case 'sensor_failure':
    ...
    async completeIrrigation(scheduleId, results) {
        try {
            ...
            if (status) {
                await scada_gate_control_service_1.scadaGateControlService.closeGate(status.fieldId);
            }
...
exports.IrrigationControllerV2Service = IrrigationControllerV2Service;
exports.irrigationControllerV2Service = new IrrigationControllerV2Service();
//# sourceMappingURL=irrigation-controller-v2.service.js.map