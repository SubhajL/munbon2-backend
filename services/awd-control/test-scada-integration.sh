...
PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -c "SELECT COUNT(*) FROM tb_site WHERE stationcode IS NOT NULL;" 2>&1 | grep -q "20"
if [ $? -eq 0 ]; then
...
PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -c "\d tb_gatelevel_command" 2>&1 | grep -q "gate_level"
if [ $? -eq 0 ]; then
...
COMMAND_ID=$(PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -t -c "
...
sleep 2
STATUS=$(PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -t -c "
