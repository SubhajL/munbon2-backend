...
PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d munbon_dev -c "SELECT version();" > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -c "SELECT COUNT(*) FROM tb_site;" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ SCADA database connection successful${NC}"
    # Show site count
    SITE_COUNT=$(PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -t -c "SELECT COUNT(*) FROM tb_site WHERE stationcode IS NOT NULL;")
...
PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d sensor_data -c "SELECT version();" > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -c "\d tb_gatelevel_command" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ tb_gatelevel_command table accessible${NC}"
    # Show recent commands
    echo -e "${YELLOW}Recent gate commands:${NC}"
    PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d db_scada -c "
...
PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d munbon_dev -c "SELECT COUNT(*) FROM awd.awd_fields;" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ AWD schema accessible${NC}"
    FIELD_COUNT=$(PGPASSWORD=__ROTATED_DB_PASSWORD__ psql -h ${EC2_HOST:-43.208.201.191} -U postgres -d munbon_dev -t -c "SELECT COUNT(*) FROM awd.awd_fields;")
