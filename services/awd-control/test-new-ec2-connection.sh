...
PGPASSWORD=P@ssw0rd123! psql -h 43.209.22.250 -U postgres -d munbon_dev -c "SELECT version();" > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
PGPASSWORD=P@ssw0rd123! psql -h 43.209.22.250 -U postgres -d db_scada -c "SELECT COUNT(*) FROM tb_site;" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ SCADA database connection successful${NC}"
    # Show site count
    SITE_COUNT=$(PGPASSWORD=P@ssw0rd123! psql -h 43.209.22.250 -U postgres -d db_scada -t -c "SELECT COUNT(*) FROM tb_site WHERE stationcode IS NOT NULL;")
...
PGPASSWORD=P@ssw0rd123! psql -h 43.209.22.250 -U postgres -d sensor_data -c "SELECT version();" > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
PGPASSWORD=P@ssw0rd123! psql -h 43.209.22.250 -U postgres -d db_scada -c "\d tb_gatelevel_command" > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
PGPASSWORD=P@ssw0rd123! psql -h 43.209.22.250 -U postgres -d munbon_dev -c "SELECT COUNT(*) FROM awd.awd_fields;" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ AWD schema accessible${NC}"
    FIELD_COUNT=$(PGPASSWORD=P@ssw0rd123! psql -h 43.209.22.250 -U postgres -d munbon_dev -t -c "SELECT COUNT(*) FROM awd.awd_fields;")
