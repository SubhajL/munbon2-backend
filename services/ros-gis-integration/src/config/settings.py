...
class Settings(BaseSettings):
    ...
    requirement_source_postgres_url: Optional[str] = Field(
        default=None,
        env="REQUIREMENT_SOURCE_POSTGRES_URL",
    )
    redis_url: str = Field(default="redis://localhost:6379/2", env="REDIS_URL")

    # External Service URLs
    flow_monitoring_url: str = Field(
        default="http://localhost:3011", env="FLOW_MONITORING_URL"
    )
    scheduler_url: str = Field(default="http://localhost:3021", env="SCHEDULER_URL")
    ros_service_url: str = Field(
        default="http://localhost:3047", env="ROS_SERVICE_URL"  # Correct ROS port
    )
    ...
    weather_api_url: str = Field(
        default="https://api.weather.example.com", env="WEATHER_API_URL"
    )

    # Mock server for development
    use_mock_server: bool = Field(default=False, env="USE_MOCK_SERVER")
    mock_server_url: str = Field(default="http://localhost:3099", env="MOCK_SERVER_URL")
    ...
    # Canonical daily requirement publication
    daily_requirement_enabled: bool = Field(
        default=False,
    ...
    daily_requirement_timezone: str = Field(
        default="Asia/Bangkok",
        env="DAILY_REQUIREMENT_TIMEZONE",
    )
    daily_requirement_horizon_days: int = Field(
        default=7,
        ge=1,
        le=31,
        env="DAILY_REQUIREMENT_HORIZON_DAYS",
    )
    daily_requirement_input_max_age_hours: int = Field(
        default=4320,
        ge=1,
    ...
    scheduler_service_url: str = Field(
        default="http://localhost:3021", env="SCHEDULER_SERVICE_URL"
    )
    ...
    def external_service_url(self) -> str:
        ...
        return (
            self.mock_server_url if self.use_mock_server else self.flow_monitoring_url
        )
