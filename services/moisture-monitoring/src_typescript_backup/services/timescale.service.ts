...
export class TimescaleService {
  ...
  async getLatestReadings(sensorIds?: string[], limit: number = 100): Promise<MoistureReading[]> {
    let query = `
      SELECT 
        sensor_id,
        time as timestamp,
  ...
  async getActiveSensors(): Promise<MoistureSensor[]> {
    const query = `
  ...
  async getAnalytics(
    ...
    const getSurfaceTrend = () => {
      if (Math.abs(row.surface_slope) < 0.01) return 'stable';
      return row.surface_slope > 0 ? 'increasing' : 'decreasing';
    };
    
    const getDeepTrend = () => {
      if (Math.abs(row.deep_slope) < 0.01) return 'stable';
    ...
    return {
      ...
      trends: {
        ...
        trendStrength: Math.max(
          parseFloat(row.surface_r2) || 0,
          parseFloat(row.deep_r2) || 0
        ),
      },
    };
  }
