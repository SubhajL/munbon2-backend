...
from clients.water_level_client import WaterLevelClient
...
class DailyDemandCalculator:
    ...
    def __init__(self):
        ...
        self.water_level_client = WaterLevelClient()
        self.cache = None
    ...
    async def calculate_daily_demands(
        ...
        for plot in plots:
            ...
            # Water level adjustments are now handled in ROS service
            # The BFF no longer applies post-processing adjustments
            # Combined demand uses the adjusted values from ROS if available
            if ros_demand and 'adjustedNetDemandM3' in ros_demand:
                # Use adjusted demand as the final combined demand
                combined_demand['net_demand_m3'] = ros_demand['adjustedNetDemandM3'] / 7  # Convert weekly to daily
                combined_demand['original_demand_m3'] = ros_demand['netWaterDemandM3'] / 7
                combined_demand['adjusted_demand_m3'] = ros_demand['adjustedNetDemandM3'] / 7
                combined_demand['water_level_m'] = ros_demand.get('weeklyAvgWaterLevel')
                combined_demand['adjustment_factor'] = ros_demand.get('waterLevelAdjustmentFactor', 1.0)
                combined_demand['adjustment_method'] = ros_demand.get('adjustmentMethod', 'ros_integrated')
                combined_demand['water_level_data_quality'] = ros_demand.get('waterLevelDataQuality')
            else:
                # No water level adjustment available
                combined_demand['original_demand_m3'] = combined_demand['net_demand_m3']
                combined_demand['adjusted_demand_m3'] = combined_demand['net_demand_m3']
                combined_demand['adjustment_factor'] = 1.0
                combined_demand['adjustment_method'] = 'no_adjustment'
            
            # Calculate AWD demand if applicable
            awd_demand = await self._calculate_awd_demand(plot, combined_demand['net_demand_m3'], date)
            ...
            daily_demands[plot_id] = {
                ...
                "awd_demand_m3": awd_demand.get('awd_demand_m3', 0),
                "combined_demand_m3": combined_demand['net_demand_m3'],
                "original_demand_m3": combined_demand.get('original_demand_m3', combined_demand['net_demand_m3']),
                "adjusted_demand_m3": combined_demand.get('adjusted_demand_m3', combined_demand['net_demand_m3']),
                "water_level_m": combined_demand.get('water_level_m'),
                "adjustment_factor": combined_demand.get('adjustment_factor', 1.0),
                "adjustment_method": combined_demand.get('adjustment_method', 'none'),
                "water_level_data_quality": combined_demand.get('water_level_data_quality'),
                ...
                "awd_enabled": awd_demand.get('awd_enabled', False),
                "awd_water_saved_m3": awd_demand.get('water_saved_m3', 0),
    ...
    async def _calculate_ros_demand(
        ...
        try:
            ...
            if self.water_level_client and plot.get('section_id'):
                try:
                    # Get aggregated weekly water level from ROS
                    weekly_level_data = await self.ros_client.get_weekly_water_level({
                        "areaId": plot['section_id'],
                        "areaType": "section",
                        "calendarWeek": date.isocalendar()[1],
                        "calendarYear": date.year
                    })
                    if weekly_level_data:
                        weekly_water_level = weekly_level_data.get('avgWaterLevelM')
            ...
            # Get water demand with water level
            demand_input = {
                "areaId": plot['plot_id'],
            ...
            if weekly_water_level is not None:
                demand_input["waterLevel"] = weekly_water_level
            
            demand = await self.ros_client.calculate_water_demand(demand_input)
    ...
    async def _calculate_awd_demand(
        self, 
        plot: Dict,
        standard_demand_m3: float,
        date: date
    ) -> Dict:
        """Calculate AWD (Alternate Wetting and Drying) demand for a plot"""
        from services.awd_integration import AWDIntegrationService
        
        awd_service = AWDIntegrationService()
        
        # Check if AWD is enabled for this plot
        awd_status = await awd_service.client.get_plot_status(plot['plot_id'])
        
        if not awd_status or not awd_status.get('is_active'):
            return {
                "awd_enabled": False,
                "awd_demand_m3": standard_demand_m3,  # Use standard demand if AWD not enabled
                "water_saved_m3": 0,
                "savings_percent": 0
            }
        
        # Get AWD parameters
        irrigation_interval = awd_status.get('irrigation_interval_days', 7)
        soil_moisture_threshold = awd_status.get('soil_moisture_threshold', 0.7)
        ponding_depth = awd_status.get('ponding_depth_cm', 5)
        
        # Calculate reduction factor based on AWD parameters
        # AWD typically saves 15-30% water
        if irrigation_interval <= 5:
            reduction_factor = 0.85  # 15% savings
        elif irrigation_interval <= 7:
            reduction_factor = 0.75  # 25% savings  
        else:
            reduction_factor = 0.70  # 30% savings
            
        # Adjust for soil moisture threshold
        reduction_factor = reduction_factor + ((1 - soil_moisture_threshold) * 0.1)
        
        # Ensure reduction factor is within reasonable bounds
        reduction_factor = max(0.6, min(0.9, reduction_factor))
        
        # Calculate AWD demand
        awd_demand_m3 = standard_demand_m3 * reduction_factor
        water_saved_m3 = standard_demand_m3 - awd_demand_m3
        savings_percent = (water_saved_m3 / standard_demand_m3) * 100 if standard_demand_m3 > 0 else 0
        
        return {
            "awd_enabled": True,
            "awd_demand_m3": round(awd_demand_m3, 2),
            "water_saved_m3": round(water_saved_m3, 2),
            "savings_percent": round(savings_percent, 2),
            "irrigation_interval_days": irrigation_interval,
            "ponding_depth_cm": ponding_depth,
            "soil_moisture_threshold": soil_moisture_threshold,
            "next_irrigation_date": awd_status.get('next_irrigation_date')
        }
            ...
            async with self.db.get_connection() as conn:
                ...
                for plot_id, demand in demands.items():
                    values.append((
                        ...
                        demand['area_rai'],
                        demand['source'],
                        demand.get('original_demand_m3'),
                        demand.get('adjusted_demand_m3', demand['combined_demand_m3']),  # Use combined if no adjusted
                        demand.get('water_level_m'),
                        demand.get('adjustment_factor', 1.0),
                        demand.get('adjustment_method', 'none'),
                        demand.get('water_level_data_quality')
                    ))
                ...
                await conn.executemany(
                    ...
                    (plot_id, section_id, date, ros_demand_m3, aquacrop_demand_m3,
                     ...
                     area_rai, source, original_demand_m3, adjusted_demand_m3,
                     water_level_m, adjustment_factor, adjustment_method,
                    ...
                    ON CONFLICT (plot_id, date) DO UPDATE SET
                        ...
                        water_level_data_quality = EXCLUDED.water_level_data_quality,
