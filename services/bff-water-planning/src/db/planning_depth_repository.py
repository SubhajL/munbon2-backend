...
from services.planning_depth_submission import (
    ...
    canonicalize_planning_depth_request_v2,
    expand_planning_depth_values,
    validate_canonical_roster,
)

LEGACY_CALENDAR_SYSTEM = "legacy-calendar-v1"
...
def _receipt_v2(row, *, replayed: bool) -> PlanningDepthSubmissionReceiptV2:
...
async def _load_active_row(
    ...
    return await connection.fetchrow(
        ...
        WHERE current.project_key = $1
          AND current.calendar_system = $2
          AND current.week_key = $3
          AND NOT EXISTS (
              SELECT 1
              ...
              WHERE successor.supersedes_submission_id = current.submission_id
                AND successor.project_key = current.project_key
                AND successor.calendar_system = current.calendar_system
                AND successor.week_key = current.week_key
          )
...
async def _create_planning_depth_submission(
    connection,
    ...
    try:
        async with connection.transaction():
            await connection.fetchval(
                ...
                (
                    f"planning-depth:{request.project_key}:"
                    f"{calendar_system}:{request.week_key}"
                ),
            )
...
async def create_planning_depth_submission(
    ...
    row, replayed = await _create_planning_depth_submission(
        ...
        principal,
        roster,
        calendar_system=LEGACY_CALENDAR_SYSTEM,
        canonicalize_request=canonicalize_planning_depth_request,
    )
    return _receipt(row, replayed=replayed)


async def create_planning_depth_submission_v2(
    connection,
    ...
    row, replayed = await _create_planning_depth_submission(
        ...
        principal,
        roster,
        calendar_system=RID_CALENDAR_SYSTEM,
        canonicalize_request=canonicalize_planning_depth_request_v2,
    )
    return _receipt_v2(row, replayed=replayed)


async def _load_active_submission(
    connection,
    project_key: str,
    calendar_system: str,
    week_key: str,
...
async def get_active_planning_depth_submission(
    ...
    active, levels = await _load_active_submission(
        connection,
        project_key,
        LEGACY_CALENDAR_SYSTEM,
        week_key,
    )
    if active is None:
        return None
...
async def get_active_planning_depth_submission_v2(
    connection,
    project_key: str,
    calendar_system: str,
    week_key: str,
) -> PlanningDepthActiveSubmissionV2 | None:
    if calendar_system != RID_CALENDAR_SYSTEM:
        raise ValueError("unsupported planning-depth calendar system")
    active, levels = await _load_active_submission(
        connection,
        project_key,
        calendar_system,
    ...
    return PlanningDepthActiveSubmissionV2(
        ...
        client_submission_id=active["client_submission_id"],
        project_key=active["project_key"],
        calendar_system=active["calendar_system"],
        week_key=active["week_key"],
        week_date=active["week_date"],
        submitted_at=active["submitted_at"],
        submitted_by=active["submitted_by"],
