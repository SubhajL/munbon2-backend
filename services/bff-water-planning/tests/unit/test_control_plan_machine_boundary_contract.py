"""BFF acceptance suite for the immutable control-plan evidence v1 contract."""
...
MODELS = {
    "intent-timeline.schema.json": ControlPlanIntentTimeline,
    "readback-observations.schema.json": ControlPlanReadbackObservations,
    "execution-state.schema.json": ControlPlanExecutionState,
}
NESTED_MODELS = {
    "intent-timeline.schema.json": {"intent": IntentTimelineEntryProjection},
    "readback-observations.schema.json": {"observation": ReadbackObservationProjection},
    "execution-state.schema.json": {"hold_event": HoldEventProjection},
}
...
def test_bff_manifest_pins_the_complete_evidence_contract_set():
...
def test_bff_models_and_json_schemas_agree_on_every_evidence_fixture():
    manifest = _load(ROOT / "manifest.json")
...
def test_bff_model_fields_match_every_evidence_schema_object():
