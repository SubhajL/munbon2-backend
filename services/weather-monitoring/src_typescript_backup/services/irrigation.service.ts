...
interface CropData {
  ...
  criticalMoisture: number; // Critical soil moisture level (%)
  fieldCapacity: number; // Field capacity (%)
  wiltingPoint: number; // Wilting point (%)
  rootDepth: number; // Root depth (cm)
}
...
export class IrrigationService {
  ...
  async getIrrigationRecommendation(
    ...
    // Check cache first
    const cached = await this.cacheService.get<IrrigationRecommendation>(cacheKey);
    ...
    const et = await this.analyticsService.calculateEvapotranspiration(
      location,
      new Date(),
      cropData.kc
    );
  ...
  async getIrrigationSchedule(
    ...
    irrigationSystem: 'drip' | 'sprinkler' | 'flood' = 'flood'
  ): Promise<any> {
    ...
    const waterRequirement = this.calculateWaterRequirement(
    ...
    const schedule = this.generateIrrigationSchedule(
      waterRequirement,
      irrigationSystem,
  ...
  async getWaterBalanceAnalysis(
    ...
    const historicalWeather = await this.databaseService.getHistoricalWeather(
      startTime,
      endTime,
      location
    );
    ...
    const dailyBalance = await this.calculateDailyWaterBalance(
      historicalWeather,
      location,
      cropData
    );
  ...
  private analyzeForecast(forecasts: WeatherForecast[]): any {
  ...
  private generateRecommendation(
    ...
    // Decision logic
    let recommendation: 'irrigate' | 'postpone' | 'reduce' | 'maintain' = 'maintain';
    ...
    // High ET and low expected rainfall - irrigate
      ...
      recommendation = 'irrigate';
      suggestedAmount = this.calculateIrrigationAmount(
        et.et0 * 7 - forecastAnalysis.totalRainfall,
        cropData.rootDepth
      );
    ...
    // Extended dry period expected - irrigate preventively
      ...
      recommendation = 'irrigate';
      suggestedAmount = this.calculateIrrigationAmount(
        et.et0 * forecastAnalysis.rainDays,
        cropData.rootDepth
      );
    ...
    const suggestedTime = this.calculateOptimalIrrigationTime(
      currentWeather,
      forecastAnalysis
    );
  ...
  private calculateIrrigationAmount(deficit: number, rootDepth: number): number {
  ...
  private async calculateDailyWaterBalance(
    weatherData: any[],
    location: { lat: number; lng: number },
    cropData: CropData
  ): Promise<any[]> {
  ...
  private groupByDay(readings: any[]): Map<string, any[]> {
  ...
  private estimateSoilMoisture(waterBalance: number, cropData: CropData): number {
    ...
    return Math.max(
      cropData.wiltingPoint,
      Math.min(cropData.fieldCapacity, baselineMoisture + adjustment)
    );
  }

  private calculateIrrigationEfficiency(dailyBalance: any[]): number {
    const totalIrrigation = dailyBalance.reduce((sum, day) => sum + day.irrigationNeeded, 0);
    const totalET = dailyBalance.reduce((sum, day) => sum + day.et, 0);
    
    if (totalIrrigation === 0) return 100;
    
    return Math.min(100, (totalET / totalIrrigation) * 100);
  }

  private suggestAdjustments(dailyBalance: any[], cropData: CropData): string[] {
  ...
  private findConsecutiveDryDays(dailyBalance: any[]): number {
  ...
  private checkExtendedDryPeriod(forecasts: WeatherForecast[]): boolean {
  ...
  private getDefaultCropData(): CropData {
    return {
      type: 'generic',
      growthStage: 'vegetative',
      kc: 1.0,
  ...
  private isRecommendationValid(recommendation: IrrigationRecommendation): boolean {
    const now = new Date();
