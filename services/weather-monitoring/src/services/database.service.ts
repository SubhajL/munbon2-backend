...
export class DatabaseService {
  ...
  constructor() {
    ...
    const timescaleConfig: PoolConfig = {
    ...
    this.timescalePool = new Pool(timescaleConfig);
    this.postgresPool = new Pool(postgresConfig);
    
    this.timescalePool.on('error', (err) => {
      logger.error({ err }, 'Unexpected error on TimescaleDB idle client');
    });
  ...
  async getCurrentWeather(location?: { lat: number; lng: number }, stationIds?: string[]): Promise<WeatherReading[]> {
    let query = `
    ...
    try {
      ...
      const readings = this.mapToWeatherReadings(result.rows, WeatherDataSource.AOS);
      ...
      logger.error({ error }, 'Failed to query TimescaleDB');
    }
    
    // Fallback to PostgreSQL (other weather sources)
    try {
      const result = await this.postgresPool.query(query, params);
      return this.mapToWeatherReadings(result.rows, WeatherDataSource.TMD);
    } catch (error) {
      logger.error({ error }, 'Failed to query PostgreSQL');
      return [];
    }
  }
  ...
  async getHistoricalWeather(
    ...
    try {
      ...
      readings.push(
        ...this.mapToWeatherReadings(timescaleResult.rows, WeatherDataSource.AOS),
        ...this.mapToWeatherReadings(postgresResult.rows, WeatherDataSource.TMD)
      );
    } catch (error) {
      logger.error({ error }, 'Failed to query historical weather');
    }
  ...
  async getWeatherStations(active?: boolean): Promise<WeatherStation[]> {
    const query = `
    ...
    try {
      ...
      stations.push(
        ...this.mapToWeatherStations(timescaleResult.rows, WeatherDataSource.AOS),
        ...this.mapToWeatherStations(postgresResult.rows, WeatherDataSource.TMD)
      );
    } catch (error) {
  ...
  async getWeatherForecasts(
    ...
    try {
      ...
      logger.error({ error }, 'Failed to get weather forecasts');
      return [];
    }
  }

  private mapToWeatherReadings(rows: any[], defaultSource: WeatherDataSource): WeatherReading[] {
    return rows.map(row => ({
  ...
  private mapToWeatherStations(rows: any[], defaultSource: WeatherDataSource): WeatherStation[] {
    return rows.map(row => ({
      stationId: row.station_id,
      name: row.name,
      type: row.type,
  ...
  private mapToWeatherForecasts(rows: any[]): WeatherForecast[] {
    return rows.map(row => ({
      stationId: row.station_id,
      location: {
        lat: parseFloat(row.location_lat),
