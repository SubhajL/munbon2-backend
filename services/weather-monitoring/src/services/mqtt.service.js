...
class MqttService extends events_1.EventEmitter {
    ...
    async connect() {
        return new Promise((resolve, reject) => {
            const options = {
                host: config_1.config.mqtt.host,
                port: config_1.config.mqtt.port,
                protocol: config_1.config.mqtt.protocol,
                username: config_1.config.mqtt.username,
                password: config_1.config.mqtt.password,
            ...
            this.client.on('error', (error) => {
                logger_1.logger.error({ error }, 'MQTT error');
            ...
            this.client.on('reconnect', () => {
                ...
                if (this.reconnectAttempts > this.maxReconnectAttempts) {
                    logger_1.logger.error('Max MQTT reconnection attempts reached');
                    this.client?.end();
                }
            });
    ...
    handleMessage(topic, message) {
        try {
            ...
            logger_1.logger.debug({ topic, payload }, 'Received MQTT message');
            ...
            logger_1.logger.error({ error, topic }, 'Failed to handle MQTT message');
        }
    }
    handleCommand(topic, payload) {
        ...
        switch (command) {
            ...
            case 'update-config':
                ...
                break;
            default:
                logger_1.logger.warn({ command }, 'Unknown command');
        }
    }
    handleRequest(topic, payload) {
        ...
        switch (request) {
            ...
            case 'irrigation':
                ...
                break;
            default:
                logger_1.logger.warn({ request }, 'Unknown request');
        }
    }
    ...
    async publishWeatherData(reading) {
        if (!this.connected || !this.client) {
            logger_1.logger.warn('Cannot publish - MQTT not connected');
            return;
        }
        ...
        return new Promise((resolve, reject) => {
            this.client.publish(topic, payload, { qos: 1, retain: false }, (err) => {
                if (err) {
                    logger_1.logger.error({ err, topic }, 'Failed to publish weather data');
                    reject(err);
                }
                else {
                    logger_1.logger.debug({ topic, stationId: reading.stationId }, 'Published weather data');
                    resolve();
                }
            });
        });
    }
    async publishWeatherAlert(alert) {
        if (!this.connected || !this.client) {
            logger_1.logger.warn('Cannot publish - MQTT not connected');
            return;
        }
        ...
        return new Promise((resolve, reject) => {
            this.client.publish(topic, payload, { qos: 2, retain: true }, (err) => {
                if (err) {
                    logger_1.logger.error({ err, topic }, 'Failed to publish weather alert');
                    reject(err);
                }
                else {
                    logger_1.logger.info({ topic, alertId: alert.id, type: alert.type }, 'Published weather alert');
                    resolve();
                }
            });
        });
    }
    async publishForecast(location, forecast) {
        if (!this.connected || !this.client) {
            logger_1.logger.warn('Cannot publish - MQTT not connected');
            return;
        }
        ...
        return new Promise((resolve, reject) => {
            this.client.publish(topic, payload, { qos: 1, retain: true }, (err) => {
                if (err) {
                    logger_1.logger.error({ err, topic }, 'Failed to publish forecast');
                    reject(err);
                }
                else {
                    logger_1.logger.debug({ topic, location }, 'Published forecast');
                    resolve();
                }
            });
        });
    }
    async publishAnalytics(location, analytics) {
        if (!this.connected || !this.client) {
            logger_1.logger.warn('Cannot publish - MQTT not connected');
            return;
        }
        ...
        return new Promise((resolve, reject) => {
            this.client.publish(topic, payload, { qos: 0, retain: false }, (err) => {
                if (err) {
                    logger_1.logger.error({ err, topic }, 'Failed to publish analytics');
                    reject(err);
                }
                else {
                    logger_1.logger.debug({ topic, location }, 'Published analytics');
                    resolve();
                }
            });
        });
    }
    async publishIrrigationRecommendation(recommendation) {
        if (!this.connected || !this.client) {
            logger_1.logger.warn('Cannot publish - MQTT not connected');
            return;
        }
        ...
        return new Promise((resolve, reject) => {
            this.client.publish(topic, payload, { qos: 1, retain: true }, (err) => {
                if (err) {
                    logger_1.logger.error({ err, topic }, 'Failed to publish irrigation recommendation');
                    reject(err);
                }
                else {
                    logger_1.logger.info({ topic, location: recommendation.location }, 'Published irrigation recommendation');
                    resolve();
                }
            });
        });
