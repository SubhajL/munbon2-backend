...
const { RidplanAgriculturalProcessor } = require('./ridplan-agricultural-processor');
...
class ShapeFileProcessor {
    ...
    async processShapeFile(options) {
        ...
        try {
            ...
            // Check if this contains data_ridplan folder
            const hasRidplan = fs.existsSync(path.join(uploadDir, 'data_ridplan'));
            if (hasRidplan) {
                logger.info('Processing as ridplan data with agricultural focus');
                const ridplanProcessor = new RidplanAgriculturalProcessor();
                return await ridplanProcessor.processShapeFile(options);
            }
    ...
    async parseShapeFiles(directory) {
        ...
        if (gpkgFiles.length > 0) {
            const allParcels = [];
            const { GeoPackageProcessor } = require('./geopackage-processor');
