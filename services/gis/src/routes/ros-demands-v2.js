...
router.post('/demands', auth_1.authenticateJWT, [
    (0, express_validator_1.body)('parcelId').optional().isUUID(),
    (0, express_validator_1.body)('sectionId').notEmpty().isString(),
    (0, express_validator_1.body)('calculationDate').notEmpty().isISO8601(),
    (0, express_validator_1.body)('calendarWeek').notEmpty().isInt({ min: 1, max: 53 }),
    (0, express_validator_1.body)('calendarYear').notEmpty().isInt({ min: 2020 }),
    (0, express_validator_1.body)('cropType').notEmpty().isString(),
    (0, express_validator_1.body)('areaRai').notEmpty().isFloat({ min: 0 }),
    (0, express_validator_1.body)('grossDemandM3').notEmpty().isFloat({ min: 0 }),
    (0, express_validator_1.body)('netDemandM3').notEmpty().isFloat({ min: 0 }),
    ...
    try {
        ...
        if (!parcelId && demand.sectionId) {
            const parcelResult = await database_1.AppDataSource.query(`SELECT id FROM gis.agricultural_plots 
           WHERE plot_code = $1 OR properties->>'externalId' = $1 
           LIMIT 1`, [demand.sectionId]);
        ...
        const result = await database_1.AppDataSource.query(query, values);
        logger_1.logger.info('ROS water demand saved', {
        ...
        logger_1.logger.error('Error saving ROS water demand', { error });
        res.status(500).json({
...
router.post('/demands/bulk', auth_1.authenticateJWT, [
    (0, express_validator_1.body)('demands').isArray({ min: 1 }),
    (0, express_validator_1.body)('demands.*.sectionId').notEmpty().isString(),
    (0, express_validator_1.body)('demands.*.calendarWeek').notEmpty().isInt({ min: 1, max: 53 }),
    (0, express_validator_1.body)('demands.*.calendarYear').notEmpty().isInt({ min: 2020 }),
], (0, async_handler_1.asyncHandler)(async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        for (const demand of demands) {
            let parcelId = demand.parcelId;
            if (!parcelId && demand.sectionId) {
                const parcelResult = await queryRunner.query(`SELECT id FROM gis.agricultural_plots 
             WHERE plot_code = $1 OR properties->>'externalId' = $1 
             LIMIT 1`, [demand.sectionId]);
                if (parcelResult.length > 0) {
                    parcelId = parcelResult[0].id;
        ...
        await database_1.AppDataSource.query('REFRESH MATERIALIZED VIEW CONCURRENTLY gis.weekly_demand_summary');
        logger_1.logger.info('Bulk ROS water demands saved', {
        ...
        logger_1.logger.error('Error saving bulk ROS water demands', { error });
        res.status(500).json({
...
router.get('/demands', auth_1.authenticateJWT, [
    (0, express_validator_1.query)('sectionId').optional().isString(),
    ...
    (0, express_validator_1.query)('calendarWeek').optional().isInt({ min: 1, max: 53 }),
    (0, express_validator_1.query)('calendarYear').optional().isInt({ min: 2020 }),
    (0, express_validator_1.query)('latest').optional().isBoolean(),
    ...
    const { sectionId, amphoe, tambon, cropType, calendarWeek, calendarYear, latest = false, page = 1, limit = 20 } = req.query;
    try {
        let query = `
        SELECT 
          rwd.*,
      ...
      `;
        ...
        const countResult = await database_1.AppDataSource.query(countQuery, values);
        ...
        logger_1.logger.error('Error fetching ROS water demands', { error });
        res.status(500).json({
...
router.get('/demands/summary', auth_1.authenticateJWT, [
    ...
    try {
      ...
      `;
        ...
        logger_1.logger.error('Error fetching demand summary', { error });
        res.status(500).json({
...
router.get('/demands/comparison', auth_1.authenticateJWT, [
    (0, express_validator_1.query)('amphoe').optional().isString(),
    (0, express_validator_1.query)('tambon').optional().isString(),
    (0, express_validator_1.query)('week').optional().isInt({ min: 1, max: 53 }),
    (0, express_validator_1.query)('year').optional().isInt({ min: 2020 }),
      ...
      `;
        ...
        res.json({
            ...
            summary: {
                ...
                avgDifferenceM3: result.reduce((sum, row) => sum + (row.weekly_difference_m3 || 0), 0) / result.length
            }
        });
    }
    catch (error) {
        logger_1.logger.error('Error fetching demand comparison', { error });
        res.status(500).json({
...
//# sourceMappingURL=ros-demands-v2.js.map