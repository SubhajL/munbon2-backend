...
in ONE transaction and recorded in scheduler.schema_migrations with a content
...
MIGRATIONS_SCHEMA_DDL = "CREATE SCHEMA IF NOT EXISTS scheduler"
...
async def _registry_exists(conn) -> bool:
    return (
        await conn.fetchval(
            "SELECT to_regclass('scheduler.schema_migrations')"
        )
        is not None
    )
...
async def apply_migration(conn, migration_id: str) -> str:
    ...
    async with conn.transaction():
        ...
        await conn.execute(
            "INSERT INTO scheduler.schema_migrations "
...
async def rollback_migration(conn, migration_id: str) -> str:
    ...
    async with conn.transaction():
        ...
        await conn.execute(
            "DELETE FROM scheduler.schema_migrations WHERE migration_id = $1",
            migration_id,
        )
        return "rolled-back"
