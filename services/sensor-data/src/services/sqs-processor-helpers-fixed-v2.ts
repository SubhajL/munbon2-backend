...
export function formatGatewayId(gwId: string): string {
...
export async function processSingleMoistureSensor(
  ...
  try {
    ...
    logger.debug({ sensorId, originalGwId: gatewayId }, 'Sensor registered/updated');
  } catch (error) {
  ...
  try {
    ...
    logger.info({ 
      sensorId, 
      originalGwId: gatewayId,
      formattedGwId: formattedGatewayId,
