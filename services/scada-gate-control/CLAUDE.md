...
- **6.1b deferred (documented):** the quantizer monotone/readback-separability invariants are enforced by the OFFLINE `validateApprovedRegistryCoverage`; the runtime 6.1a `loadDeviceCapabilitySnapshot` still only shape-validates, so a hand-authored (non-CLI-generated) registry isn't re-checked at load. When real D6 data + the 6.3 readback reconciler land, add a startup self-check of those invariants on the served snapshot. Also deferred: a randomized TS↔Python `canonicalize` differential test (today the reachable surface is ASCII-only ids, guarded by the 6.0/6.1a golden vectors).
