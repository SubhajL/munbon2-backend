...
export function SystemPanel({ summary, error }: SystemPanelProps) {
  return (
    ...
    >
      ...
      <p className="mb-3 text-xs text-fg-muted">RID Munbon Phase 2 — Gate Control</p>
      {error ? (
        <div
          role="alert"
        ...
        >
          <AlertTriangle aria-hidden className="mt-0.5 size-4 shrink-0" style={{ color: 'var(--color-offline)' }} />
          <span>
            เชื่อมต่อ API ไม่ได้ (Cannot reach the gate API). {error}. กำลังลองใหม่อัตโนมัติ
