...
describe('ConfirmCommandModal', () => {
  test('renders the title, body and raw Modbus details when open', () => {
    render(
      <ConfirmCommandModal open gateName="Waste Way" level={2} onConfirm={() => {}} onCancel={() => {}} />,
    );
    expect(screen.getByRole('dialog')).toHaveTextContent('ยืนยันการสั่งงานประตูน้ำ');
    expect(screen.getByText('ต้องการสั่ง Waste Way ไปที่ เปิดระดับ 1 ใช่หรือไม่?')).toBeInTheDocument();
  ...
  test('confirm and cancel buttons fire their handlers', async () => {
    ...
    render(
      <ConfirmCommandModal open gateName="Waste Way" level={3} onConfirm={onConfirm} onCancel={onCancel} />,
    );
  ...
  test('renders no dialog when closed', () => {
    render(
      <ConfirmCommandModal open={false} gateName="Waste Way" level={2} onConfirm={() => {}} onCancel={() => {}} />,
    );
