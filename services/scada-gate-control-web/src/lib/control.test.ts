...
describe("currentLevelInfo", () => {
  test("describes the current open level and flow", () => {
    expect(currentLevelInfo(2)).toBe(
      "ระดับประตูน้ำปัจจุบัน: เปิดระดับ 1 อัตราไหลน้ำ = 0.5 ลบ.ม./วินาที",
    );
  });
});
...
describe("confirmBody", () => {
  test("names the gate and the target level", () => {
    expect(confirmBody("Waste Way", 3)).toBe(
      "ต้องการสั่ง Waste Way ไปที่ เปิดระดับ 2 ใช่หรือไม่?",
    );
  });
});
