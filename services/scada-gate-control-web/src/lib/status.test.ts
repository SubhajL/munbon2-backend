...
import { connectionLabel, STATUS_COLOR_VAR, summarizeConnections } from './status';
...
describe('summarizeConnections', () => {
  test('counts zero for an empty list', () => {
    expect(summarizeConnections([])).toEqual({ online: 0, stale: 0, offline: 0 });
  });

  test('buckets each marker colour into the right counter', () => {
    expect(
      summarizeConnections([site('green'), site('green'), site('yellow'), site('red')]),
