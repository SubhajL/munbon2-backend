  ...
  apps: [
    ...
    // Sensor Data Service - Full TypeScript microservice for ingestion
    {
      ...
      env: {
        ...
        VALID_TOKENS:
          "munbon-ridr-water-level:water-level,munbon-m2m-moisture:moisture",
        EXTERNAL_API_KEYS:
          "rid-ms-dev-1234567890abcdef,test-key-fedcba0987654321",
        // MQTT settings
    ...
    // Cloudflare Tunnel for External API - Provides legacy TLS/cipher support
    {
      ...
      post_deploy:
        'sleep 5 && grep -o "https://[a-zA-Z0-9\\-]*\\.trycloudflare\\.com" ./logs/tunnel-external-out.log | tail -1 > ./services/sensor-data/tunnel-external-url.txt',
    },
    ...
    // Sensor Location Mapping Service
    {
      ...
      env: {
        ...
        TIMESCALE_DB: process.env.TIMESCALE_DB || "sensor_data",
        ...
        POSTGIS_PORT: process.env.POSTGIS_PORT || 5432,
        POSTGIS_DB: process.env.POSTGIS_DB || "gis_db",
    ...
    // Sensor Data Consumer - Processes sensor data from SQS
    {
      ...
      env: {
        ...
        SQS_QUEUE_URL:
          "https://sqs.ap-southeast-1.amazonaws.com/108728974441/munbon-sensor-ingestion-dev-queue",
        // Consumer port
        CONSUMER_PORT: 3004,
      },
    ...
    // SCADA Integration Service
    {
      ...
      env: {
        PORT: 3015,
        NODE_ENV: "development",
        SCADA_DB_HOST: process.env.SCADA_DB_HOST,
        SCADA_DB_PORT: process.env.SCADA_DB_PORT || 5432,
        SCADA_DB_NAME: process.env.SCADA_DB_NAME,
        SCADA_DB_USER: process.env.SCADA_DB_USER,
        SCADA_DB_PASSWORD: process.env.SCADA_DB_PASSWORD,
      },
